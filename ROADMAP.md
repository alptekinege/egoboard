...
**Revision 2026-09-25 (P2-A dashboard):** Delivered the usage dashboard first in the P2 sequence (read-only aggregates, QPainter fallback, toolbar + `>dashboard`, `tst_dashboard` 8 slots). No schema change; no new optional dep. Nothing committed.
...
- **P2-A — Usage dashboard ✅ delivered 2026-09-25.** Read-only local stats over existing indexes: counts by day/app/type, top apps, size histogram, streak; custom `QWidget`/`QPainter` fallback (no QtCharts dependency, no bundled assets; `QPalette` colors, `tr()` strings) — `DashboardStats::collect()` (fast indexed COUNT path on `StorageManager`, bounded 200-row `fetchPage` path otherwise; no `fetchAllFull`, no VACUUM) + `DashboardDialog` (summary + day/type/size bar charts + top-apps list, empty-DB empty-state, keyboard arrows/Home/End, AT names, `Reduce motion` by construction, sensitive counts labeled without content). Opens from toolbar (More off Wide) + `>dashboard` (aliases `stats`/`usage`) + cheatsheet row; in-app privacy hint + release-checklist X11/Wayland + QtCharts-absent matrix row (README/`docs/build.md` deferred to P2 end per scope). 50k-entry aggregates ~24 ms offscreen (budget 500 ms). Tests: `tst_dashboard` 8 slots (fixture, streak edges, buckets, empty state, offscreen render + keyboard, `>dashboard` parse, contrast over installed schemes, 2000-row < 500 ms) + 35/35 + `--smoke` OK.
