...
2. **R5 settings and onboarding:** complete U14 settings search, per-page reset, responsive narrow layout, storage status/quota/integrity presentation, and named profiles (settings JSON portability and dialog search delivered 2026-09-23); complete U15 first-run tour, shortcut cheatsheet, and last-capture tray guidance. Retain the delivered thread-safe diagnostics and crash-report flows (U19/U20).
...
- 2026-09-23 progress (search delivered): search box atop the dialog filters the sidebar (translated labels + harvested knob texts, no keyword table) and bolds matching knobs with contrast-safe weight-only highlight, match-count hint, pre-search row restore, re-filter after deferred population; 3 new `tst_uidesign` slots — full suite 30/30 + `--smoke` OK. Remaining: per-page reset, responsive narrow layout, profiles.
...
| **R5 — Settings & onboarding** ◐ *partial (U19/U20 landed 2026-09-22)* | Settings search + reset + profiles + responsive dialog; settings JSON portability ✅ 2026-09-23; first-run tour; cheatsheet; Settings crash hardening ✅ and crash-report toolkit ✅ | U14, U15, **U19** ✅, **U20** ✅ |
...
> R0–R2 are sequential (shell before surfaces). R3–R6 can reorder by need — each ships independently. **U11/U12/U13/U19 (P0), U17/U18/U20 (P1) are delivered**; remaining: U14/U15 (Settings search, profiles, tour), U10 accessibility tail, §8 platform items.
...
*Last updated: 2026-09-23 (status synced: U1–U13, U16–U20 delivered; R4 complete; U14 partial — portability + search done; U15 open, U10 partial) · Next: U14 (per-page reset, responsive narrow, profiles), U15 (tour, cheatsheet, tray guidance), or U10/R6 tails.*
