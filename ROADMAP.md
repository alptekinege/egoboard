...
**Revision 2026-09-23 (U11 trash/undo, partial):** Delivered the U11 undo half: additive `trash_entries`/`trash_entry_tags`/`trash_entry_groups` tables (idempotent `IF NOT EXISTS`, legacy DBs gain them on open) with `softDeleteEntries` / `softClearHistory` / `expireEntriesToTrash` / `restoreTrashEntries` / `purgeTrash` / `trashCount` on `StorageManager` (GUI-free, FTS-trigger consistent, dedup-conflict safe); MainWindow deletes + clear-history and the expire sweep restore exact IDs via toast Undo (5 s window, no payload snapshots); expiry shows a toast when the window is up, the notification otherwise; 8 new `tst_storage` slots — 30/30 pass + `--smoke` OK + `--bench=100000` within budgets. Cancelable backup/import/export/restore progress (G9) remains open. Previous revision notes preserved below.
...
1. **R4 feedback and states:** complete U11 undo semantics — trash-based undo for deletes, clear-history and expiry sweeps delivered 2026-09-23 (exact-ID restore, no snapshot spikes); remaining: cancellable progress for backup/import/export/restore, and non-blocking loading placeholders (U12 motion and U13 empty/error states delivered 2026-09-23). Keep `Reduce motion` and accessibility behavior throughout.
...
- 2026-09-23 progress (undo half delivered): trash tables + `StorageManager` trash API + MainWindow delete/clear rewiring (exact-ID restore, no snapshot spikes) + expire-sweep undo toast via `ExpireScheduler::takeLastExpiredIds()` (toast when the window is visible, `KNotification` otherwise); 1 h retention purged at open and on soft-delete; 8 new `tst_storage` slots (round-trip, links/payloads, hash-live drop, purge bound, soft-clear, expire-to-trash, signals, legacy migration) — full suite 30/30 + `--smoke` OK + `--bench=100000` within budgets. Remaining: cancelable progress for backup/import/export/restore (G9).
...
| **R4 — Feedback & states ◐ partial** | Skeletons, empty-state/toast/motion foundations, capture sound+notification, bulk image export, shared motion, empty/error states and trash-based undo delivered; cancelable-progress polish remains | U11, **U12** ✅, **U13** ✅, **U16** ✅, **U17** ✅ |
| **R5 — Settings & onboarding** ◐ *partial (U19/U20 landed 2026-09-22)* | Settings search + reset + profiles + responsive dialog; first-run tour; cheatsheet; Settings crash hardening ✅ and crash-report toolkit ✅ | U14, U15, **U19** ✅, **U20** ✅ |
...
> R0–R2 are sequential (shell before surfaces). R3–R6 can reorder by need — each ships independently. **U12/U13/U19 (P0), U17/U18/U20 (P1) are delivered**; remaining: U14/U15 (Settings search, profiles, tour), R4 cancelable progress (U11/G9), §8 platform items.
...
*Last updated: 2026-09-23 (status synced: U1–U9, U12/U13, U16–U20 delivered; U11 partial — trash/undo done, cancelable progress open; U14/U15 open) · Next: U11 cancelable progress (G9) or U14/U15 (Settings search + reset + profiles, first-run tour).*
